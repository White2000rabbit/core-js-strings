...
function sumOfCodes(str) {
  const array = str.split('').map((item) => item.charCodeAt());
  const endsum = array.reduce((sum, elem) => sum + elem, 0);
  return endsum;
  if str =''
}
...
function isPalindrome(str) {
  const strno = str.replaceAll(' ', '').toLowerCase();
